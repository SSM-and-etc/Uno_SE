...
class Title():
    def __init__(self, main):
        ...
        self.select_state = -1  # 0: single game start, 1: option, 2: exit ...
    ...
    def display(self, main):
        if self.on_option:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.USEREVENT and self.ex_key_counter > -1:
                    self.ex_key_counter -= 1
    ...
    def move_collide_title(self, main, mouse_pos):
        if (self.single_game_rect.collidepoint(mouse_pos)):
            self.select_state = -1
        elif (self.single_game_rect.collidepoint(mouse_pos)):
            self.select_state = 0
        elif (self.story_mode_rect.collidepoint(mouse_pos)):
            self.select_state = 1
        elif (self.option_rect.collidepoint(mouse_pos)):
    ...
    def apply_state_change(self):
        ...
        self.button_select_pos = self.tup_mul(
            screen_size, self.button_select_default_poses[self.select_state])
        self.button_select_rect = self.button_select_img.get_rect(
            center=self.button_select_pos)

    def enter_state(self, main):
        if (self.select_state == STATE_SINGLE_GAME):
            main.scene_change(main.get_scene_index("single game"))
        if (self.select_state == STATE_MULTI_GAME):
    ...
    def set_gui_default_poses(self):
        ...
        self.button_select_default_poses = [
            (0.2, 0.65), (0.4, 0.65), (0.6, 0.65), (0.8, 0.65)]

        self.default_ex_key_text_pos = (0.35, 0.5)

    def set_gui_poses(self, screen_size):
        self.single_game_pos = self.tup_mul(
            screen_size, self.single_game_default_pos)
        self.option_pos = self.tup_mul(screen_size, self.option_default_pos)
        self.story_mode_pos = self.tup_mul(
            screen_size, self.story_mode_default_pos)
        self.exit_pos = self.tup_mul(screen_size, self.exit_default_pos)
        self.button_select_pos = self.tup_mul(
            screen_size, self.button_select_default_poses[self.select_state])

        self.ex_key_text_pos = self.tup_mul(
            screen_size, self.default_ex_key_text_pos)

    def set_gui_imges(self, screen_size):
        scale_ratio = self.tup_div(screen_size, self.design_size)
        self.title_BG = pygame.transform.scale(self.default_title_BG, self.tup_mul(
            self.get_img_size(self.default_title_BG), scale_ratio))
        self.single_game_img = pygame.transform.scale(self.default_single_game_img, self.tup_mul(
            self.get_img_size(self.default_single_game_img), scale_ratio))
        self.option_img = pygame.transform.scale(self.default_option_img, self.tup_mul(
            self.get_img_size(self.default_option_img), scale_ratio))
        self.story_mode_img = pygame.transform.scale(self.default_story_mode_img, self.tup_mul(
            self.get_img_size(self.default_story_mode_img), scale_ratio))
        self.exit_img = pygame.transform.scale(self.default_exit_img, self.tup_mul(
            self.get_img_size(self.default_exit_img), scale_ratio))
        self.button_select_img = pygame.transform.scale(self.default_button_select_img, self.tup_mul(
            self.get_img_size(self.default_button_select_img), scale_ratio))
        self.button_select_CM_img = pygame.transform.scale(self.default_button_select_CM_img, self.tup_mul(
            self.get_img_size(self.default_button_select_img), scale_ratio))

    def set_gui_rct(self):
        self.single_game_rect = self.single_game_img.get_rect(
            center=self.single_game_pos)
        self.option_rect = self.option_img.get_rect(center=self.option_pos)
        self.story_mode_rect = self.story_mode_img.get_rect(
            center=self.story_mode_pos)
        self.exit_rect = self.exit_img.get_rect(center=self.exit_pos)
        self.button_select_rect = self.button_select_img.get_rect(
            center=self.button_select_pos)

    def set_text(self, screen_size):
        font_ratio = (screen_size[0]/self.design_size[0] +
                      screen_size[1]/self.design_size[1]) / 2
        font = pygame.font.SysFont("arial", int(
            self.default_font_size * font_ratio), True, True)

        ex_key_text = "Left: " + pygame.key.name(self.user_data.key_left) + " , Right: " + pygame.key.name(
            self.user_data.key_right) + ", Enter: " + pygame.key.name(self.user_data.key_enter)
        self.ex_key_text = font.render(ex_key_text, True, BLUE_MAGENTA)

    def load_asset(self, root):
        self.default_title_BG = pygame.image.load(
            os.path.join(root, "Material/BG/title.png"))
        self.default_single_game_img = pygame.image.load(
            os.path.join(root, "Material/Button/single_game.png"))
        self.default_option_img = pygame.image.load(
            os.path.join(root, "Material/Button/option.png"))
        self.default_story_mode_img = pygame.image.load(
            os.path.join(root, "Material/Button/story_mode.png"))
        self.default_exit_img = pygame.image.load(
            os.path.join(root, "Material/Button/exit.png"))
        self.default_button_select_img = pygame.image.load(
            os.path.join(root, "Material/Button/button_select.png"))
        self.default_button_select_CM_img = pygame.image.load(
            os.path.join(root, "Material/ColorMode/colormode_button_select.png"))
